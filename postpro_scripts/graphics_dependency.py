...
from mpl_toolkits.mplot3d import Axes3D  # noqa: F401 - kept for legacy compatibility
import numpy as np
...
def tuple_cmp(a, b):
    """Legacy comparator retained for callers that import it directly."""
    if a[0] <= b[0]:
        return -1
    return 1
...
def _partition_ids_by_quantile(ID_score, num, frac=None):
    IDscore = [[ID, score] for ID, score in ID_score.items()]
    IDscore = sorted(IDscore, key=lambda item: item[1])
    if frac is None:
    ...
    for i in range(num):
        ...
        st = ed
        ed = st + size
        output.append([IDscore[j][0] for j in range(st, ed)])
    return output


def _sorted_ids_by_position(ID_pos):
    return sorted([[pos, ID] for ID, pos in ID_pos.items()], key=lambda item: item[0])


def _smooth_signal(values, win):
    return signal.fftconvolve(values, np.ones((win,)) / win, mode='same')


def _get_or_create_ax(ax, fig_width, fig_height, **subplot_kwargs):
    if ax is not None:
        return ax, False
    _, ax = plt.subplots(figsize=(fig_width, fig_height), **subplot_kwargs)
    return ax, True


def _finalize_figure(make_fig, save, show, save_path, filename, fmt=None, dpi=None, tight=True, close=True):
    if not make_fig:
        return
    if save:
        ...
        if dpi is not None:
            save_kwargs['dpi'] = dpi
        plt.savefig(save_path + filename, **save_kwargs)
    if show:
        if tight:
            plt.tight_layout()
        plt.show()
...
def _filter_keys(key_values, keys=None):
    if keys:
        return [key for key in keys if key in key_values]
    return list(key_values.keys())


def _mask_profile_edges(profile, pad_len):
    if pad_len <= 0:
        return profile
...
def _legend_handles(leg):
    if hasattr(leg, 'legendHandles'):
        return leg.legendHandles
    if hasattr(leg, 'legend_handles'):
        return leg.legend_handles
    return leg.get_lines()


def _set_legend_marker_style(leg, markersize=15, alpha=1):
    for handle in _legend_handles(leg):
        try:
            handle._legmarker.set_markersize(markersize)
            handle._legmarker.set_alpha(alpha)
        except Exception:
            try:
...
# -----------------------------------------------------------------------------
# Legacy graphics.py API


def quantile(ID_score, num, frac=None):
    return _partition_ids_by_quantile(ID_score, num, frac=frac)
...
def moving_average(signal, win):
    ...
    for i in range(half_win, len(signal) - half_win):
        value = sum(signal[i - half_win:i + half_win + 1]) / float(win)
...
def draw_along_genome(ID_pos, ID_sig_list, win, labels, ylabel, title=None, xlim=None, ylim=None, scatt=False, note=''):
    posID = _sorted_ids_by_position(ID_pos)
    X = []
    Y_list = [[] for _ in range(len(ID_sig_list))]
    for pos, ID in posID:
        X.append(pos)
        for i in range(len(ID_sig_list)):
            ID_sig = ID_sig_list[i]
            Y_list[i].append(ID_sig[ID])
    plt.figure(figsize=(10, 5))
    if scatt:
        for Y in Y_list:
            plt.plot(X, Y, '.', color='orange', alpha=0.2)
    for i in range(len(Y_list)):
        Y = _smooth_signal(Y_list[i], win)
...
def draw_along_genome_pair(ID_pos, ID_sig1, ID_sig2, win, ylabel1, ylabel2, title='', xlabel='Genomic coordinate (bp)', note=''):
    ...
    for pos, ID in posID:
        ...
        Y1.append(sig1)
        Y2.append(sig2)
    Y1 = _smooth_signal(Y1, win)
    space1 = int(len(Y1) * 0.05)
    Y1[:space1] = np.nan
    Y1[len(Y1) - space1:len(Y1)] = np.nan
    Y2 = _smooth_signal(Y2, win)
    space2 = int(len(Y2) * 0.05)
    Y2[:space2] = np.nan
    Y2[len(Y2) - space2:len(Y2)] = np.nan
...
def Scatter_plot(ID_xvalue, ID_score, xlim=[0, 100], ylim=[-2, 2.5], markersize=None, xlabel='AT content (%)', ylabel='Condensability (A.U.)', note=''):
    ...
    for ID in ID_xvalue:
        ...
        X.append(xvalue)
        Y.append(score)
        xvalue_scores.setdefault(xvalue, []).append(score)
    Xmean, Ymean, Yerr = [], [], []
    for xvalue in xvalue_scores:
        scores = xvalue_scores[xvalue]
        if len(scores) <= 1:
            continue
        Xmean.append(xvalue)
        Ymean.append(np.mean(scores))
...
def PartitionBoxplot(ID_score, ID_target, frac, xlabel='', ylabel='Condensability (A.U.)', title='', note=''):
    frac = sorted(_normalize_fraction(frac), reverse=True)
...
def PartitionMeanplot(ID_xvalue, ID_score, ID_target, frac, xlim=[0, 100], ylim=[-0.8, 0.6], xlabel='AT content (%)', ylabel='Condensability (A.U.)', note=''):
    ...
    for i in range(len(group)):
        ...
        for ID in IDs:
            xvalue = ID_xvalue[ID]
            xvalue_scores.setdefault(xvalue, []).append(ID_score[ID])
        X, Y, Z = [], [], []
        for xvalue in xvalue_scores:
            scores = xvalue_scores[xvalue]
            if len(scores) <= 1:
...
def plot_boxplot (key_values,
    ...
    keys = _filter_keys(key_values, keys)
...
def plot_boxplot_pair (key_values1,
    ...
    if keys:
        keys = [key for key in keys if key in key_values1 and key in key_values2]
    else:
        keys = list(set(key_values1.keys()) & set(key_values2.keys()))
...
def plot_boxplot_multiple (key_values_list,
    ...
    if keys:
        keys = [key for key in keys if key in common_keys]
    else:
        keys = common_keys
...
def plot_profile (profile,
    ...
    profile = _mask_profile_edges(profile, pad_len)
...
def plot_profile_pair (profile1,
    ...
    profile1 = _mask_profile_edges(profile1, pad_len)
    profile2 = _mask_profile_edges(profile2, pad_len)
